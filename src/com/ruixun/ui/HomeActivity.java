...
public class HomeActivity extends BaseActivity
	...
	public void onRest(BaseProtocol protocol) {
		...
		if (protocol instanceof DeviceList) {
			...
			ArrayAdapter<Device> adapter = new ArrayAdapter<Device>(this, android.R.layout.simple_spinner_dropdown_item,
	...
	public void registerComplete(final String name, final String pwd) {
		restManager.register(name, MD5.getMD5(pwd),
				/* appManager.getCurrentDevice().getId() */deviceSelected.getId(), new RestListener() {
	...
	protected void goMsgInfoActivity() {
		...
		if (appManager.getUser().getStatus() == 1) {
			...
			// 进入第一个信息查询界面
			MessageInfoActivity_.intent(HomeActivity.this).start();
		finish();
	}

	protected void goDeviceListActivity() {
		DeviceListActivity_.intent(HomeActivity.this).start();
	}
	...
	public void loginComplete(final String name, final String pwd) {
		restManager.login(name, MD5.getMD5(pwd),
				/* appManager.getCurrentDevice().getId() */deviceSelected.getId(), new RestListener() {
	...
	public void callService() {
		...
		Intent phoneIntent = new Intent(
				"android.intent.action.CALL" /* Intent.ACTION_DIAL */, Uri.parse("tel:" + Constants.BASE_TELEPHONE));
